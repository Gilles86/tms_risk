...
def main(subject, bids_folder):
    ...
    epi = op.join(bids_folder, 'derivatives', 'fmriprep', f'sub-{subject:02d}', f'ses-1', 'func',
            f'sub-{subject:02d}_ses-1_task-task_run-1_space-T1w_boldref.nii.gz')
    ...
    pycortex_transform.save(f'tms.sub-{subject:02d}', 'epi', xfmtype='coord')

    identity_transform = Transform(np.identity(4), epi).save(f'tms.sub-{subject:02d}', 'epi.identity')
