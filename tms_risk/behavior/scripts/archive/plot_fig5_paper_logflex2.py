"""Figure 5 for logflex2, in the fig5.flexible2nf design (exploratory draft).
...
def nu(n, first, vertex):
    ...
    bp = model.make_dm(nc, variable='perceptual_noise_sd').astype(np.float32)
    cp = SC[('perceptual', 'Intercept')]
    if vertex:
        cp = cp + SC[('perceptual', VC)]
    pre = np.einsum('sdj,gj->sdg', cp, bp)
    if first:
        bm = model.make_dm(nc, variable='memory_noise_sd').astype(np.float32)
...
for row, (rf_bool, oname) in enumerate(ORDERS):
    ...
    # --- E ----------------------------------------------------------------
    ax = axes[row, 4]
    e_all = pd.read_csv(DATA / 'ppc_by_safe_delta.logflex2.tsv', sep='\t')
...
fig.suptitle('Figure 5 draft — logflex2 (subject-averaged); contours: vertex '
             'P(risky); dotted: risk-neutral ratio', fontsize=8, color='.35',
             y=1.04)
sns.despine(fig=fig, offset=3)
fig.savefig(OUT / 'fig5_paper_logflex2.pdf')
fig.savefig(OUT / 'fig5_paper_logflex2.png', dpi=150)
print('wrote', OUT / 'fig5_paper_logflex2.pdf')
