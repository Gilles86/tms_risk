...
**UPDATE 2026-08-19: all six models are now cross-validated, and CV does NOT establish
m5.** See `notes/cvr2_model_set_handoff_2026-08-19.md` for the full table
(`notes/data/cvr2_vs_null_m0-5.tsv`, SLURM 5103983). On `NPCr2cm-cluster`, m1 wins
outright (Δ vs null +0.0131, t(34) = 2.67, p = 0.011; best in 26/35 subjects; beats every
other model pairwise at p ≤ 0.003). m5 sits at +0.0086 over the null (p = 0.088) and is
statistically indistinguishable from m4 (Δ = +0.0010, p = 0.27) — so out-of-sample fit
cannot adjudicate tuning vs magnitude, and the parameter-level evidence above remains the
only argument. The ordering across the set is a clean per-session-parameter complexity
penalty: 1 free (m1) > 2 (m3, m5, m4) > 4 (m2) > 0 (m0).
`plot_encoding_model_comparison.py` is unblocked (reads
`notes/data/cvr2_model_grid.tsv`, currently derived from the m0-5 TSV — per-voxel
`frac_wins` still needs the extractor).

Read the cvR² numbers against the **null of −0.0178 on this mask** (`cvr2_vs_null.py`
recomputes it per voxel under the identical folds): cvR² = 0 is not the null, because
`get_rsq` puts the held-out fold's own mean in the denominator.
